...
public static class ZhTwTextCleaner
{
    ...
    private static readonly Dictionary<string, string> Words =
        new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            { "Artifacts", "神器" }, { "Baetyls", "貝提爾" }, { "Lairs", "巢穴" },
{ "Merchants", "商人" }, { "Oddities", "珍奇之物" },
    ...
    private static readonly Dictionary<string, string> PhraseLeaks =
        new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            { "Settle the dispute in Bey Lah and doom the village.", "解決貝拉(Bey Lah)的紛爭，並毀滅整個村莊。" },  
            { "Settle the dispute in Bey Lah in Eskhind's favor.", "解決貝拉(Bey Lah)的紛爭，讓埃斯欣德(Eskhind)得利。" },  
            { "Settle the dispute in Bey Lah in Hindriarch Keh's favor.", "解決貝拉(Bey Lah)的紛爭，讓牧母凱(Keh)得利。" },  
            ...
            { "D-Pad Right", "方向鍵 右" },
            { "D-Pad Left", "方向鍵 左" },
            { "I know that everything's all right", "我知道一切都會沒事的" },
            { "right through", "直直穿過" },
            { "right past", "正好經過" },
            { "[=left= sec]", "[剩餘 =left= 秒]" },
            { "I'm left with no recourse except to grieve.", "我別無選擇，只能悲慟。" },
            { "Set Checkpoint", "設定儲存點" },
            { "Set Terse messages", "設定簡短訊息" },
            { "Set Verbose messages", "設定詳細訊息" },
            { "Set target temperature.", "設定目標溫度。" },
            { "Set Primary Limb", "設定主肢" },
            { "Set Bomb", "設置炸彈" },
            { "This body part cannot be set as your primary.", "此肢體部位無法設定為主肢。" },
            { "Pronoun Set: ", "代名詞組合：" },
            { "Bomb Set", "炸彈套件" },
            { "Factionrep set to all hated.", "派系聲望設定為全體敵視。" },
            { "Factionrep set to all loved.", "派系聲望設定為全體友好。" },
            { "+2 DV while wielding a long blade in the primary hand.", "主手持長刃時 +2 DV。" },
            { "+2 to hit while wielding a long blade in the primary hand.", "主手持長刃時 +2 命中。" },
            { "+2/3 DV while wielding a long blade in your primary hand.", "主手持長刃時 +2/3 DV。" },
            { "+2/3 to hit while wielding a long blade in your primary hand.", "主手持長刃時 +2/3 命中。" },
            { "+3 DV while wielding a long blade in the primary hand.", "主手持長刃時 +3 DV。" },
            { "+3 to hit while wielding a long blade in the primary hand.", "主手持長刃時 +3 命中。" },
            { "It looks like a long way down ", "看起來距離下方很遠 " },
            { "Long Sword3", "長劍3(Long Sword3)" },
            { "Long Sword4", "長劍4(Long Sword4)" },
            { "not have enough charge to operate", "沒有足夠的電量運作" },
            { "not have enough charge to function", "沒有足夠的電量運作" },
            { "Not Enough Charge", "電量不足" },
            { "Current charge", "目前電量" },
            { "Maximum charge", "最大電量" },
            { "You need at least =amount= charge to generate a discharge.", "你需要至少 =amount= 電量才能產生放電。" },
            { "You can't charge more than ", "你不能充電超過 " },
            { "Charge left after last turn", "回合結束後剩餘電量" },
            { "Adjacent charge", "相鄰傳電" },
            { "High-capacity: This item has increased charge capacity.", "高容量：此物品的電量容量增加。" },
            { "Discharge [=charge= charge]", "放電 [=charge= 電量]" },
            { "Pick a filter to change", "選擇要變更的篩選條件" },
            { "Pick item", "選擇物品" },
            { "Pick Target", "選擇目標" },
            { "Pick step from ", "從 選擇步驟" },
            { "per turn", "每回合" },
            { "per turn to ", "每回合 " },
            { "Turn Music On", "開啟音樂" },
            { "Turn Music Off", "關閉音樂" },
            { "(light) -30% Quickness", "(輕載) -30% 敏捷" },
            { "Lanterned: This item provides light.", "裝燈特性：此物品提供照明。" },
            { "Torch Light Fail (covered)", "火把照明失敗（覆蓋）" },
            { "Torch Light Fail (submerged)", "火把照明失敗（浸沒）" },
            { "turn cooldown", "回合冷卻" },
            { "more turn", "更多回合" },
            { "spend a turn", "花費一回合" },
            { "a turn.", "一回合。" },
    ...
    private static readonly Dictionary<string, string> ProperNounZh =
        new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            { "Eskhind", "埃斯欣德(Eskhind)" }, { "Baetyl", "貝提爾(Baetyl)" },
            { "Lithofex", "石蝶(Lithofex)" }, { "Decarbonizer", "除碳器(Decarbonizer)" },
            { "Timereaver", "時光掠奪者(Timereaver)" }, { "Golgotha", "髑髏地(Golgotha)" }, 
    ...
    private static readonly Dictionary<string, string> CommonWords =
        new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            { "hollow", "谷地" }, { "ruins", "遺跡" }, { "stilt", "高蹺" }, { "gate", "門" },
    ...
    private static readonly Dictionary<string, string> TmpWords =
        new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            // 屬性
            { "Strength", "力量" }, { "Agility", "敏捷" }, { "Toughness", "韌性" },
            { "Willpower", "意志力" }, { "Intelligence", "智力" }, { "Ego", "心智" },
            { "Speed", "速度" }, { "MoveSpeed", "移動速度" }, { "Level", "等級" }, { "Third", "第三" },
            // 技能/能力（高頻白名單）
            { "Swift Reflexes", "迅捷反射" }, { "Spry", "靈活" }, { "Jump", "跳躍" },
            { "Tumble", "翻滾" }, { "Axe Proficiency", "斧頭精通" }, { "Cleave", "劈砍" },
            { "Charging Strike", "衝鋒打擊" }, { "Dismember", "肢解" },
            { "Hook and Drag", "勾引與拖曳" }, { "Decapitate", "斬首" }, { "Berserk!", "狂暴！" },
            { "Cudgel Proficiency", "棍棒熟練度" }, { "Bludgeon", "鈍器打擊" }, { "Conk", "撞擊" },
            { "Backswing", "後擺" }, { "Slam", "猛擊" }, { "Demolish", "拆除" },
            { "Flurry", "狂亂" }, { "Lunge", "突刺" }, { "Swipe", "揮擊" },
            { "Dueling Stance", "決鬥架勢" }, { "Block", "格擋" }, { "Shield Slam", "盾牌撞擊" },
            { "Deft Blocking", "靈巧格擋" }, { "Swift Blocking", "迅捷格擋" },
            { "Staggering Block", "踉蹌格擋" }, { "Shield Wall", "盾牌牆壁" },
            { "Jab", "刺擊" }, { "Hobble", "跛行" }, { "Shank", "刺殺" },
            { "Hurdle", "跨越" }, { "Deft Throwing", "靈巧投擲" }, { "Charge", "衝鋒" },
            { "Kickback", "反作用力" }, { "Juke", "閃躲" }, { "Akimbo", "雙持" },
            { "Disassemble", "拆解" }, { "Reverse Engineer", "逆向工程師" },
            { "Scavenger", "拾荒者" }, { "Repair", "修理" }, { "Deploy Turret", "部署砲塔" },
            { "Tinker I", "修補匠 I" }, { "Tinker II", "修補匠 II" }, { "Tinker III", "修補匠 III" },
            { "Meditate", "冥想" }, { "Fasting Way", "禁食之道" }, { "Iron Mind", "鋼鐵意志" },
            { "Lionheart", "獅心" }, { "Conatus", "內驅力" }, { "Mind over Body", "心勝於身" },
            { "Proselytize", "勸誘信徒" }, { "Intimidate", "威嚇" }, { "Berate", "斥責" },
            { "Snake Oiler", "蛇油商" }, { "Inspiring Presence", "鼓舞人心的存在感" },
            { "Menacing Stare", "威脅凝視" }, { "Steady Hands", "穩定的手" },
            { "Make Camp", "紮營" }, { "Mind's Compass", "心靈指南針" },
    ...
    private static readonly Dictionary<string, string> SentenceDict =
        ...
        { "I found the village stores of Bey Lah untouched; Eskhind must have traded something for her basics.", "我發現貝拉(Bey Lah)的村莊商店未經觸動；埃斯欣德(Eskhind)一定是拿了某樣東西來交換她的生活必需品。" },
    ...
    private static readonly Dictionary<string, string> EffectZh =
        new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            { "glitching", "故障中" }, { "freezing", "結冰中" }, { "frozen", "凍結" },
