...
{ "Merchants", "商人" }, { "Oddities", "珍奇之物" },
    ...
    private static readonly Dictionary<string, string> PhraseLeaks =
        new Dictionary<string, string>(StringComparer.OrdinalIgnoreCase)
        {
            ...
            { 'Recovers 0.6 hit points per level (minimum 3) each turn.', '每回合回復生命值，每級 0.6（最低 3）。' },  
            { 'Recovers 0.9 hit points per level (minimum 5) each turn.', '每回合回復生命值，每級 0.9（最低 5）。' },  
            ...
            { 'Save As...', '另存為……' },  
            { 'Save file is less than two bytes.', '儲存檔小於兩個位元組。' },  
            { 'Save file is the incorrect version.', '儲存檔版本不正確。' },
            ...
            { 'You activate the recoiler.', '你啟動了歸返器。' },  
            ...
            { 'You are stuck in a remote pocket dimension and cannot recoil out.', '你被困在偏遠的口袋次元中，無法用歸返器離開。' },  
            ...
            { "You can't recoil with hostiles nearby!", '附近有敵對者時你不能使用歸返器！' },  
            { "You can't recoil yet.", '你現在還不能用歸返器。' },  
            ...
            { 'You do not have any recoilers.', '你沒有任何歸返器。' },  
